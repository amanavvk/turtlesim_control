...
class turtle():
	...
	def step_vel(self,v_final,v_initial,time_sent):
		...
		if(v_initial.linear.x>self.v_max):								#velocity should not exceed v_max
...
#This function plans the future points, and moves to them.	
	def plan(self,target):
		...
		if(self.euclidean_distance(target)<3):								#check if target is in range
			print('My Position:',self.position.x,self.position.y)    		
			print('Target Position:',target.x,target.y)
			zero_vel=Twist()
			self.vel_pub.publish(zero_vel)								#Stop PT immediately in case
			print("Distance less than 3. Caught RT\n")
			self.rt_pose.unregister()
			rospy.set_param('caughtStatus',True)							#param used to stop RT also.
		else:
			predicted_pos=Pose()
			if(np.shape(a)[0]<3):
				a.append([target.x,target.y])							#Need minimum of 3 points to define circle	
			print(a)
			if(np.shape(a)[0]>=3):
				t1=time.time()
				...
				while(True):
					...
					if(self.distance(next_point)/self.v_max>(5*plan_length)):		#If point not reachable in this time slot, finds next point(5s steps)
						plan_length=plan_length+1
						a.append(next_point)
						next_point=self.next_point(a[-1],a[-2],center,radius)		#Find future points till a reachable point in timeslot is found
						...
						t2=time.time()
						self.goal_reach(predicted_pos)
						
						print("Time elapsed in planning:",t2-t1)
						break
