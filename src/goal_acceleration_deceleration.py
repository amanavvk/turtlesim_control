...
class turtle():
	def __init__(self):
		rospy.init_node('goal_navigation_one_with_a_d')
		self.position=Pose()
	...
	def goal_reach(self):	
		...
		Kd=0.1
		Ki=0.0001
		dist_prev=(self.position.x-float(goal.x))*(self.position.x-float(goal.x))+(self.position.y-float(goal.y))*(self.position.y-float(goal.y))
		...
		while(True):
			...
			vel.angular.z=4*Kp*(atan2(float(goal.y) - self.position.y, float(goal.x)- self.position.x)-self.position.theta)	#Angle needed to turn
